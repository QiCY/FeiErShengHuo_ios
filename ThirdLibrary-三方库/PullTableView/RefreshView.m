...
//        [self addSubview:self.dateLabel];
        ...
        if (self.isAtTop){
            self.arrowLayer.frame = CGRectMake(MainW/2-16,self.frame.size.height - 35, 32, 32);
            UIImage *arrow = [UIImage imageNamed:@"refresh_down"];
            self.arrowLayer.contents = (id)arrow.CGImage;
        }else{
            self.arrowLayer.frame = CGRectMake(MainW/2-16,5, 32, 32);
            UIImage *arrow = [UIImage imageNamed:@"refresh_up"];
            self.arrowLayer.contents = (id)arrow.CGImage;
        }
        self.arrowLayer.contentsGravity = kCAGravityResizeAspect;
        self.arrowLayer.contents = (id)[UIImage imageWithCGImage:[UIImage imageNamed:@"refresh_down"].CGImage scale:1 orientation:UIImageOrientationDown].CGImage;
...
//        [self addSubview:self.activityView];
        ...
        self.activityView = [[UIImageView alloc] initWithFrame:CGRectMake(MainW/2-16, self.frame.size.height - 35, 32, 32)];
        if (!self.isAtTop) {
            self.activityView.frame = CGRectMake(MainW/2-16, 5, 32, 32);
        }
        _activityView.image = [UIImage imageNamed:@"refresh"];
...
//    self.dateLabel.frame          = dateFrame;
    ...
    self.activityView.frame       = CGRectMake(arrowFrame.origin.x, arrowFrame.origin.y, 32, 32);
    self.arrowLayer.transform     = CATransform3DIdentity;
}
