...
@implementation FEHomeLimitBuyCell
...

{
    ...
    self.limitdescribeLab=[MYUI createLableFrame:CGRectZero backgroundColor:[UIColor clearColor]  text:@"" textColor:[UIColor colorWithHexString:@"#9c9c9c"] font:[UIFont systemFontOfSize:12] numberOfLines:0 adjustsFontSizeToFitWidth:NO];
    ...
    [self.goRobBtn addTarget:self action:@selector(gocarClick) forControlEvents:UIControlEventTouchUpInside];
...
-(void)setUpCellWithModel:(FEGoodModel *)model
{
    ...
    // 抢购价
            CGFloat Fprice=model.marketprice/100.0;
    ...
    //  原价
    CGFloat OriginPrice=model.productprice/100.0;
...
-(void)layoutSubviews
{
    ...
    [_limitNameLab makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.limitImageView.right).offset(5);
    ...
    [_originLab makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.limitPriceLab.right).offset(-2);
        make.top.equalTo(self.limitNameLab.bottom).offset(17);
