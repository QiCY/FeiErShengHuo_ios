...
@interface FEFishViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation FEFishViewController

#pragma mark - 懒加载数据源数组 -
//- (NSMutableArray *)LeftDataArrM {
//    if (!_LeftDataArrM) {
//        _LeftDataArrM = [[NSMutableArray alloc]init];
//    }
...
//    if (!_RightDataArrM) {
//        _RightDataArrM = [[NSMutableArray alloc]init];
//    }
...
-(void)initView
{
    self.LeftDataArrM = [[NSMutableArray alloc]init];
    self.RightDataArrM = [[NSMutableArray alloc]init];
    [self doRequest];
    self.title=@"生鲜果蔬";
    _selectIndex = 0;
    _isScrollDown = YES;
...
-(void)doRequest
{
    ...
    [dic setObject:[NSNumber numberWithInt:1] forKey:@"supplyFlag"];
    ...
    [[FEBSAFManager sharedManager]doOperationRequestHttpWithMethod:POST withPath:str withDictionary:dic withSuccessBlock:^(NSDictionary *dic) {
        ...
        [self.LeftDataArrM removeAllObjects];
        [self.RightDataArrM removeAllObjects];
