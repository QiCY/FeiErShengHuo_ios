...
#define kURLActionCount 2
NSString * const kURLActions[] = {@"at->",@"poundSign->"}; //@"url->",@"email->",@"phoneNumber->",
...
//    [self initPlist];
    ...
    NSMutableDictionary *mutableActiveLinkAttributes =@{}.mutableCopy;
    [mutableActiveLinkAttributes setObject:[NSNumber numberWithBool:NO] forKey:(NSString *)kCTUnderlineStyleAttributeName];
    
    NSMutableDictionary *mutableInactiveLinkAttributes =@{}.mutableCopy;
    [mutableInactiveLinkAttributes setObject:[NSNumber numberWithBool:NO] forKey:(NSString *)kCTUnderlineStyleAttributeName];
    
    UIColor *commonLinkColor = GLOBAL_COLOR_LAN;
    
    //点击时候的背景色
    [mutableActiveLinkAttributes setValue:(__bridge id)[[UIColor grayColor] CGColor] forKey:(NSString *)kTTTBackgroundFillColorAttributeName];
...
- (void)setEmojiText:(NSString*)emojiText
{
    ...
    if (self.isNeedAtAndPoundSign) {
        [kPoundSignRegularExpression() enumerateMatchesInString:[mutableAttributedString string] options:0 range:stringRange usingBlock:^(NSTextCheckingResult *result, __unused NSMatchingFlags flags, __unused BOOL *stop) {
            ...
            //添加链接
            NSString *actionString = [NSString stringWithFormat:@"%@%@",@"poundSign->",[self.text substringWithRange:result.range]];
        ...
        //检测名字哦
//        if (self.userModelArray.count) {
//            NSString *fString = [mutableAttributedString string];
//            for (RYUserInfoModel *curUser in self.userModelArray) {
//                if (curUser.realName.length) {
//                    NSRange nrange = [fString rangeOfString:curUser.realName];
//                    if (nrange.location != NSNotFound) {
//                        //添加链接
//                        NSString *actionString = [NSString stringWithFormat:@"%@%@",@"at->",[self.text substringWithRange:nrange]];
//                        NSTextCheckingResult *aResult = [NSTextCheckingResult correctionCheckingResultWithRange:nrange replacementString:actionString];
//                        [results addObject:aResult];
//                    }
//                }
//            }
